...
export const chatApi = {
  // 내 채팅방 목록
  getMyRooms: () =>
    api.get<ChatRoomSummary[]>('/api/chat/rooms/my').then((r) => r.data),
  ...
  getUnreadCount: (roomId: number) =>
    api
      .get<UnreadCountResponse>(`/api/chat/rooms/${roomId}/unread`)
      .then((r) => r.data),

  // 전체 미읽은 수 - 백엔드가 { totalUnread: number } 반환
  getTotalUnread: () =>
    api.get<{ totalUnread: number }>('/api/chat/unread/total').then((r) => r.data.totalUnread),

  // 채팅방 나가기
  leaveRoom: (roomId: number) =>
    api.delete(`/api/chat/rooms/${roomId}/leave`),

  // 1:1 채팅방 조회/생성 - 백엔드가 { roomId: number } 반환
  getOrCreateDirectRoom: (targetUserId: number) =>
    api
      .post<{ roomId: number }>('/api/chat/rooms/direct', null, { params: { otherId: targetUserId } })
