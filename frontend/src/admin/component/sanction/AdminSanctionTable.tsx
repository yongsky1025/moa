...
import AdminSanctionStateBadge from './AdminSanctionStateBadge';
...
const HEADERS = ['No.', '대상 유저', '제재', '상태', '대상', '기간', ''];

export default function AdminSanctionTable() {
  const navigate = useNavigate();
  const { data, loading, error, params, actualTotalPage, handlePageChange } = useAdminSanctions();
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-moa-border">
              ...
              <tr>
                <td colSpan={HEADERS.length} className="px-5 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-moa-subtle text-sm">제재 내역이 없습니다.</span>
                  </div>
                </td>
              </tr>
            )}
            ...
            {!loading &&
              list.map((s: SanctionResponseDTO, idx: number) => {
                const no = totalCount - (current - 1) * (params.size ?? 20) - idx;
                return (
                  <tr
                    key={s.sanctionId}
                    onClick={() => navigate(`/admin/sanctions/${s.sanctionId}`)}
                  ...
                  >
                    ...
                    <td className="text-moa-secondary px-5 py-3.5 whitespace-nowrap text-xs font-mono">{String(s.sanctionType)}</td>
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <AdminSanctionStateBadge state={s.sanctionState} />
                    </td>
                    <td className="text-moa-secondary px-5 py-3.5 whitespace-nowrap text-xs font-mono">
                      {String(s.targetType)} #{s.targetId}
                    </td>
                    <td className="text-moa-subtle px-5 py-3.5 whitespace-nowrap text-xs font-mono">
