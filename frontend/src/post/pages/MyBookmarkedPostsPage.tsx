...
import { Activity, Bookmark, Inbox, Loader2, Users } from "lucide-react";
...
function toDetailPath(item: MyPostItem): string {
  const { post } = item;
  if (item.source === "circle" || item.source === "activity") {
    if (post.circleId != null && post.boardId != null) {
      return `/circle/${post.circleId}/board/${post.boardId}/posts/${post.postId}`;
    }
    return "/circle";
  }
  if (post.boardType === "NOTICE") return postRoutes.noticeDetail(post.postId);
  return postRoutes.freeDetail(post.postId);
}
...
export default function MyBookmarkedPostsPage() {
  ...
  const [boardPosts, setBoardPosts] = useState<MyPostItem[]>([]);
  const [activityPosts, setActivityPosts] = useState<MyPostItem[]>([]);
  const [myCircles, setMyCircles] = useState<CircleResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const fetchCommunityBookmarkedPosts = async (): Promise<MyPostItem[]> => {
    const res = await postApi.getMyCommunityBookmarkedPosts({ board: "all" });
    return res.data
      .filter((post) => post.boardType === "FREE" || post.boardType === "NOTICE")
      .map((post) => ({ source: "community" as const, post }));
  };

  const fetchCircleBookmarkedPosts = async (): Promise<MyPostItem[]> => {
    const circlesRes = await circleApi.getMyCircles();
    if (circlesRes.data.length === 0) return [];

    const postsByCircle = await Promise.allSettled(
      circlesRes.data.map((circle) => circleBoardApi.getMyBookmarkedPosts(circle.circleId)),
    );

    return postsByCircle.flatMap((result) => {
      if (result.status !== "fulfilled") return [];
      return result.value.data.map((post) => ({ source: "circle" as const, post }));
    });
  };
      ...
      postApi
        ...
        .then((res) => {
          const sorted = res.data
            .map((post) => ({ source: "activity" as const, post }))
            .sort(
              (a, b) =>
                new Date(b.post.createDate).getTime() -
                new Date(a.post.createDate).getTime(),
            );
          setActivityPosts(sorted);
        })
        .catch(() => setError("찜한 모임 활동 게시글을 불러오는 중 오류가 발생했습니다."))
        .finally(() => setLoading(false));
      return;
    }

    Promise.all([fetchCommunityBookmarkedPosts(), fetchCircleBookmarkedPosts()])
      .then(([community, circle]) => {
        ...
        setBoardPosts(merged);
      })
      .catch(() => setError("내가 찜한 글을 불러오는 중 오류가 발생했습니다."))
      .finally(() => setLoading(false));
  }, [topTab]);

  const counts = useMemo(
    () => ({
      all: boardPosts.length,
      community: boardPosts.filter((item) => item.source === "community").length,
      circle: boardPosts.filter((item) => item.source === "circle").length,
    }),
    [boardPosts],
  );
  ...
  const displayedPosts = useMemo(() => {
    if (topTab === "activity") {
      if (activityCircleId === "all") return activityPosts;
      return activityPosts.filter((item) => item.post.circleId === activityCircleId);
    }
    if (boardFilter === "community") {
      return boardPosts.filter((item) => item.source === "community");
    }
    if (boardFilter === "circle") {
      return boardPosts.filter((item) => item.source === "circle");
    }
    return boardPosts;
            ...
            <aside className="w-44 shrink-0">
              <nav className="sticky top-24 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                {myCircles.map((circle) => {
                  const count = activityPosts.filter(
                    (item) => item.post.circleId === circle.circleId,
                  ).length;
                  return (
                ...
                {displayedPosts.map((item) => (
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2 text-xs font-semibold text-gray-400">
                        ...
                        {item.source === "community"
                          ? item.post.boardType === "NOTICE"
                            ? "커뮤니티 · 공지"
                            : "커뮤니티 · 자유"
                          : item.source === "circle"
                            ? `모임 · ${item.post.circleName ?? "모임"}`
                            : `모임 활동 · ${item.post.circleName ?? "모임"}`}
                      </div>
                      <span className="text-xs text-gray-400">
                        {new Date(item.post.createDate).toLocaleDateString("ko-KR")}
                      </span>
                    </div>
                    ...
                    <p className="mt-1 text-xs text-gray-400">
                      조회 {item.post.viewCount} · 댓글 {item.post.replyCount} · 좋아요{" "}
                      {item.post.likeCount}
                    </p>
                  </Link>
                ))}
