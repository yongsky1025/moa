...
export default function MyPostsPage() {
  ...
  const fetchCirclePosts = async (): Promise<MyPostItem[]> => {
    ...
    const postsByCircle = await Promise.allSettled(
      circlesRes.data.map((circle) => circleBoardApi.getMyPosts(circle.circleId)),
    );
  ...
  useEffect(() => {
    ...
    if (topTab === "activity") {
      postApi
        ...
        .catch(() => setError("내 모임 활동 게시글을 불러오는 중 오류가 발생했습니다."))
        .finally(() => setLoading(false));
      return;
    }

    Promise.all([fetchCommunityPosts(), fetchCirclePosts()])
      ...
      .catch(() => setError("내가 쓴 게시글을 불러오는 중 오류가 발생했습니다."))
      .finally(() => setLoading(false));
  }, [topTab]);
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
                ...
                <p className="mt-1 text-xs text-gray-400">
                  게시글을 작성하면 이곳에서 확인할 수 있습니다.
                </p>
                <button
                  type="button"
                  onClick={() => navigate("/board?board=free")}
