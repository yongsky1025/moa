...
const isFileUrl = (c: string) =>
  c.startsWith("/uploads/") || c.startsWith("/api/chat/files/");
      ...
      {replyTo && (
        <div
          style={{
            ...
            borderLeft: "3px solid #5F8F7B",
            flexShrink: 0,
          }}
        >
          <div style={{ flex: 1, overflow: "hidden" }}>
            <div style={{ fontSize: 11, color: "#5F8F7B", fontWeight: 600 }}>
              {replyTo.nickname}에게 답장
            </div>
            <div
              style={{
                fontSize: 12,
      ...
      {aiSuggestions && (
        <div style={s.aiSuggestWrap}>
          <div style={s.aiSuggestHeader}>
            <span style={{ fontSize: 11, color: "#5F8F7B", fontWeight: 600 }}>
              ✨ AI 스마트 답변
            </span>
            <button style={s.aiCloseBtn} onClick={() => setAiSuggestions(null)}>
              ✕
            </button>
          </div>
...
export default function ChatPopupPage() {
  ...
  const [roomCtxMenu, setRoomCtxMenu] = useState<{
    x: number;
    y: number;
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      const newW = Math.max(
        180,
        Math.min(480, sidebarDragStartW.current + delta),
      );
  ...
  useEffect(() => {
    ...
    chatApi
      .getReadStatus(activeRoom.roomId)
      .then((list) => {
    ...
    if (activeRoom.roomType === "GROUP" && activeRoom.circleId) {
      ...
      chatApi
        .getRoomMembers(activeRoom.roomId)
  ...
  useEffect(() => {
    if (shouldScrollToUnreadRef.current) {
      ...
      const el = msgAreaRef.current?.querySelector<HTMLElement>(
        "[data-first-unread]",
      );
      if (el) {
  ...
  const handleRenameConfirm = async () => {
    ...
    try {
      ...
      setRooms((prev) =>
        prev.map((r) =>
          r.roomId === renaming.roomId ? { ...r, name: newName } : r,
        ),
      );
      if (activeRoom?.roomId === renaming.roomId)
        setActiveRoom((prev) => (prev ? { ...prev, name: newName } : prev));
    } catch {
      alert("이름 변경 실패");
    }
    setRenaming(null);
          ...
          r.roomId === msg.roomId
            ? {
                ...
                unreadCount:
                  ...
                  mutedRoomsRef.current.has(r.roomId)
                    ? 0
                    : r.unreadCount + 1,
              }
            : r,
  ...
  const handleReadEvent = useCallback(
    (event: { userId: number; lastReadAt: string }) => {
  ...
  const handleNotification = useCallback(
    (noti: import("../../types/notification").Notification) => {
      setNotifications((prev) => {
        if (noti.id != null && prev.some((n) => n.id === noti.id)) return prev;
        return [noti, ...prev];
      });
      if (
        noti.type === "CHAT_MESSAGE" &&
        noti.referenceId &&
        !mutedRoomsRef.current.has(noti.referenceId)
      ) {
  ...
  const handleSystemEvent = useCallback(
    (event: {
      ...
      const text =
        event.type === "LEAVE"
          ? `${event.nickname}님이 퇴장했습니다.`
          : event.type === "KICK"
            ? `${event.nickname}님이 강퇴되었습니다.`
            : event.type === "RENAME"
              ? `${event.nickname}님이 방 이름을 '${event.newName}'(으)로 변경했습니다.`
  ...
  const { sendMessage, sendTyping } = useWebSocket({
    ...
    onRoomNameChange: ({ name }) => {
      setRooms((prev) =>
        prev.map((r) => (r.roomId === activeRoom?.roomId ? { ...r, name } : r)),
      );
    },
  });
  ...
  const roomLabel = (r: ChatRoomSummary) => {
    ...
    if (r.roomType === "SCHEDULE")
      return r.name ?? `일정 채팅 #${r.scheduleId}`;
  ...
  const handleDeleteMsg = async (messageId: number) => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.messageId === messageId ? deleted : m)),
      );
    } catch {
      alert("삭제 실패");
    }
  ...
  const confirmEditMsg = async (messageId: number) => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.messageId === messageId ? updated : m)),
      );
    } catch {
      alert("수정 실패");
    }
  ...
  const handleRoomNameSave = async () => {
    ...
    try {
      ...
      setRooms((prev) =>
        prev.map((r) => (r.roomId === activeRoom.roomId ? updated : r)),
      );
    } catch {
      alert("이름 변경 실패");
    }
  ...
  const handleNotiClick = async (
    n: import("../../types/notification").Notification,
  ) => {
    if (!n.isRead) {
      ...
      `}</style>
        ...
        {errorMsg && (
          <div style={s.errorToast}>
            {errorMsg}
            <button style={s.errorClose} onClick={() => setErrorMsg(null)}>
              ✕
            </button>
          </div>
        )}
        {/* ── 사이드바 ── */}
        <div
          style={{ ...s.sidebar, width: sidebarWidth, position: "relative" }}
        >
          {/* 사이드바 헤더 */}
          <div style={s.sideHeader}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              >
                ...
                >
                  <Bell size={16} />
                  {!showNoti && unreadNoti > 0 && (
                    <span style={s.dot}>{unreadNoti}</span>
                  )}
                </button>
                {showNoti && (
                  <div style={s.notiDropdown}>
                    ...
                    {chatNotifications.length === 0 ? (
                      ...
                      chatNotifications.map((n) => (
                        <div
                          key={n.id}
                          className="chat-noti-item"
                          style={{
                            ...s.notiItem,
                            background: n.isRead ? "#F8FAF9" : "#EAF4F0",
                            borderLeft: n.isRead
          ...
          <div style={s.searchWrap}>
            ...
            >
              ...
              />
              <input
                style={{ ...s.searchInput, paddingLeft: 32 }}
                placeholder="채팅방 검색"
                value={search}
                  ...
                  <div style={s.roomMeta}>
                    ...
                    <div style={s.roomTop}>
                      ...
                      {mutedRooms.has(r.roomId) ? (
                        ...
                        r.unreadCount > 0 && (
                          <span style={s.unreadBadge}>{r.unreadCount}</span>
                        )
                      )}
                    </div>
                  </div>
          ...
          >
            {roomCtxMenu.room.roomType === "GROUP" && (
              <button
                ...
                onClick={() => {
                  setRenaming({
                    roomId: roomCtxMenu.room.roomId,
                    value: roomCtxMenu.room.name ?? roomLabel(roomCtxMenu.room),
                  });
                  setRoomCtxMenu(null);
                }}
              >
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Pencil size={14} /> 방 이름 변경
                </span>
              </button>
            )}
            <button
            ...
            <button
              ...
              style={s.ctxItem}
              onClick={() => {
                toggleMute(roomCtxMenu.room.roomId);
                setRoomCtxMenu(null);
              }}
            >
              <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {mutedRooms.has(roomCtxMenu.room.roomId) ? (
                  <>
                    <Bell size={14} /> 알림 켜기
                  </>
                ) : (
          ...
          createPortal(
            ...
            >
              ...
              >
                ...
                >
                  <div
                    style={{ fontWeight: 700, fontSize: 17, color: "#262626" }}
                  >
        ...
        <div style={s.chat}>
          {!activeRoom ? (
            <div style={s.placeholder}>
              ...
              <span style={{ marginTop: 12, color: "#aaa", fontSize: 14 }}>
                채팅방을 선택하세요
              </span>
            </div>
          ) : (
              ...
              <div style={s.chatHeader}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...s.headerBtn,
                      background: showSearch ? "#EAF4F0" : "none",
                      border: "none",
                      display: "flex",
                      alignItems: "center",
                      color: showSearch ? "#5F8F7B" : "#9CA3AF",
                    }}
                    title="메시지 검색"
                  >
                    <Search size={16} />
                ...
                <div style={s.memberPanel}>
                  {members.map((m) => (
                    <div
                      ...
                      style={{
                        ...s.memberItem,
                        cursor: m.userId !== userId ? "pointer" : "default",
                      }}
                      onClick={(e) => {
                    ...
                    >
                      ...
                      {(m.role === "LEADER" || m.isLeader) && (
                        <span style={s.leaderTag}>방장</span>
                      )}
                    </div>
                  ))}
                </div>
                ...
                {loadingMsg ? (
                  ...
                  (searchQuery.trim()
                    ...
                    return (
                      <div
                        key={msg.messageId}
                        {...(firstUnreadMsgIdRef.current === msg.messageId
                          ? { "data-first-unread": "true" }
                          : {})}
                        style={{
                          ...s.msgRow,
                        ...
                        >
                          {!mine && (
                            <span style={s.senderName}>
                              {msg.senderNickname ?? `사용자 #${msg.senderId}`}
                            </span>
                          )}
                          <div
                            style={{
                            ...
                            {editingMsgId === msg.messageId ? (
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditMsgContent(e.target.value)
                                  }
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter")
                                      confirmEditMsg(msg.messageId);
                                    if (e.key === "Escape")
                                      setEditingMsgId(null);
                                  }}
                                  autoFocus
                                ...
                                style={{
                                  ...
                                  padding:
                                    ...
                                    msg.replyToId
                                      ? 0
                                      : "8px 12px",
                                  overflow: msg.replyToId
                                    ? "hidden"
                                    : undefined,
                                  boxShadow:
                                    isFileUrl(msg.content) && !msg.isDeleted
                                      ? "none"
                              ...
                              >
                                ...
                                >
                                  {msg.isReported
                                    ...
                                    : msg.isDeleted
                                      ? "삭제된 메시지입니다."
                                      : renderMsgContent(msg.content, mine)}
                                      ...
                                      activeRoom?.roomType !== "DIRECT" && (
                                        <button
                                          ...
                                          onClick={() => {
                                            setMenuId(null);
                                            window.open(
                                              `/report-form?targetType=CHAT&targetId=${msg.messageId}`,
                                              "_blank",
                                              "width=420,height=600,resizable=no",
                                            );
                                          }}
                                        >
                                          신고
                            ...
                            {mine &&
                              ...
                              (() => {
                                const unread = Object.entries(
                                  ...
                                  ([uid, time]) =>
                                    Number(uid) !== msg.senderId &&
                                    new Date(time) < new Date(msg.createdAt),
                                ).length;
                                return unread > 0 ? (
                                  <span style={s.unreadCount}>{unread}</span>
                                ) : null;
                              })()}
                            ...
                            >
                              {msg.reactions.map((r) => (
                                <button
                                  key={r.emoji}
                                  onClick={() =>
                                    handleToggleReaction(msg.messageId, r.emoji)
                                  }
                                  style={{
                                    background: r.myReaction
                                      ? "#EAF4F0"
