...
export default function FloatingChatWindow({ open, onClose }: Props) {
  ...
  const [pos, setPos] = useState({
    x: window.innerWidth - INIT_W - 40,
    y: window.innerHeight - INIT_H - 60,
  });
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      const newW = Math.max(
        140,
        Math.min(340, sidebarDragStartW.current + delta),
      );
  ...
  const { sendMessage, sendTyping } = useWebSocket({
    ...
    onRoomNameChange: ({ name }) => {
      setRooms((prev) =>
        prev.map((r) => (r.roomId === activeRoomId ? { ...r, name } : r)),
      );
    },
  });
  ...
  const handleRoomNameSave = async () => {
    ...
    try {
      ...
      setRooms((prev) =>
        prev.map((r) =>
          r.roomId === activeRoom.roomId ? { ...r, name: newName } : r,
        ),
      );
    } catch {
      alert("이름 변경 실패");
  ...
  const handleRenameConfirm = async () => {
    ...
    try {
      ...
      setRooms((prev) =>
        prev.map((r) =>
          r.roomId === renaming.roomId
            ? { ...r, name: renaming.value.trim() }
            : r,
        ),
      );
    } catch {
        ...
        createPortal(
          ...
          >
            ...
            >
              ...
              <div
                style={{ borderTop: "1px solid #E5E7EB", padding: "12px 20px" }}
              >
                <button
                  style={{
                    width: "100%",
        ...
        <div style={s.titleBar} onMouseDown={onDragMouseDown}>
          ...
          >
            ...
            <div style={{ position: "relative" }}>
              ...
              >
                <Bell size={16} fill="rgba(255,255,255,0.85)" />
                {!showNoti && unreadNoti > 0 && (
                  <span style={s.nBadge}>{unreadNoti}</span>
                )}
              </button>
              {showNoti && (
                <div style={s.notiBox}>
            ...
            <button
              className="chat-title-btn"
              style={{ ...s.titleBtn, display: "flex", alignItems: "center" }}
              onClick={() => {
                openPopup();
                onClose();
                ...
                filteredRooms.map((r) => (
                  <div
                    ...
                    style={{
                      ...s.roomItem,
                      background:
                        r.roomId === activeRoomId ? "#EAF4F0" : "transparent",
                    }}
                    ...
                    (searchQuery.trim()
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {!mine && (
                              <span style={s.nick}>{msg.senderNickname}</span>
                            )}
                            <div
                              style={{
                                display: "flex",
                ...
                {unreadOnEnter > 0 && (
                  ...
                  >
                    <button
                      onClick={() => {
                        const el =
                          msgAreaRef.current?.querySelector<HTMLElement>(
                            "[data-first-unread]",
                          );
                        if (el)
                          el.scrollIntoView({
