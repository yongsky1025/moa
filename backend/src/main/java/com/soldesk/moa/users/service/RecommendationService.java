...
public class RecommendationService {
    ...
    public RecommendationBundleDTO recommend(Long userId, int limit) {
        ...
        List<RecommendationResponseDTO> overall = profiles.stream()
                .map(ep -> new RecommendationResponseDTO(ep.getCircle(),
                        weightedEuclideanSimilarity(userOverall, toOverallVector(ep), WEIGHTS_OVERALL)))
        ...
        List<RecommendationResponseDTO> social = profiles.stream()
                .map(ep -> new RecommendationResponseDTO(ep.getCircle(),
                        weightedEuclideanSimilarity(userSocial, toSocialVector(ep), WEIGHTS_SOCIAL)))
                .sorted((a, b) -> Double.compare(b.getSimilarity(), a.getSimilarity()))
                .limit(limit)
                .collect(Collectors.toList());
        ...
        List<RecommendationResponseDTO> activity = profiles.stream()
                .map(ep -> new RecommendationResponseDTO(ep.getCircle(),
                        weightedEuclideanSimilarity(userActivity, toActivityVector(ep), WEIGHTS_ACTIVITY)))
                .sorted((a, b) -> Double.compare(b.getSimilarity(), a.getSimilarity()))
                .limit(limit)
                .collect(Collectors.toList());

        return new RecommendationBundleDTO(overall, social, activity);
    }

    // === 벡터 변환 ===

    // 유저 프로필 → 5축 전체 벡터 [socialLoad, interactionMode, activityIntensity, commitmentLevel, structureLevel]
    private double[] toOverallVector(UsersEnergyProfile profile) {
        return new double[] {
    ...
    // 서클 → 5축 전체 벡터
    private double[] toOverallVector(CircleEnergyProfile circle) {
        return new double[] {
    ...
    private double[] toSocialVector(UsersEnergyProfile profile) {
        return new double[] {
                profile.getSocialLoad(),
                profile.getInteractionMode()
        };
    ...
    // 서클 → 2축 사회적 에너지 벡터
    private double[] toSocialVector(CircleEnergyProfile circle) {
        return new double[] {
                circle.getSocialLoad(),
                circle.getInteractionMode()
        };
    ...
    private double[] toActivityVector(UsersEnergyProfile profile) {
        return new double[] {
                profile.getActivityIntensity(),
                profile.getCommitmentLevel(),
                profile.getStructureLevel()
    ...
    // 서클 → 3축 활동/몰입 벡터
    private double[] toActivityVector(CircleEnergyProfile circle) {
        return new double[] {
                circle.getActivityIntensity(),
                circle.getCommitmentLevel(),
                circle.getStructureLevel()
